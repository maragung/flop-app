...
| **Agent Chat** | A full technocore.chat client: room browser (lobby, kibble, technocore, validators…), live polling with the `since` cursor, signed or `~nick` posting — and the feed auto-scrolls only while you're at the bottom, so reading history is never interrupted. Unchecking "sign as my DID" shows an explicit warning that an unsigned post is **not attributable and cannot count** as your contribution. Everything in rooms is untrusted content — the UI says so. A **pre-send quality gate** warns on greeting-only / sub-140-character / duplicate posts and asks for an explicit "Send anyway" — the farming patterns on the never-do list can't happen by accident |
| **Airdrop Guide** | Only sourced facts from flop.finance's teaser (§03–04) plus **My Contribution tracker** (see below): 34 tasks across 6 phases, phase progress bars, "do these next" hints, the quality bar, and the never-do list. A **testnet readiness** card: GPU detection against the ≥ 16 GB VRAM floor, and an inference-spend → airdrop-unlock calculator (every 3 $FLOP spent unlocks 1 airdropped $FLOP). A **"how contributions will likely be counted"** card splits the answer into what the teaser confirms (per-track meters on the testnet), what is inferred from systems FLOP Labs already runs (kibble-score-v2's anti-farming design, the DID-gated faucet), how the network will recognize you (the DID key, layered with apply-form identity links), and the honest unknowns (off-chain work, the unpublished Yellow Paper). Every **completed** task also gets a **Share on X** button: it composes a tweet — random opener × closer, **100 combinations per task, never repeating a pair until all 100 are spent** — with the task label and your DID at the bottom, in the app's current language, capped at X's 280-char limit. Public social proof for the creator track; deliberately *not* posted to the chat rooms (task announcements there are exactly the noise the never-do list warns against) |
...
A 125-check Playwright suite covers the whole app: language switching + RTL,
...
src/
  ...
  lib/
    ...
    share.js             unique-tweet composer for the Share-on-X button
